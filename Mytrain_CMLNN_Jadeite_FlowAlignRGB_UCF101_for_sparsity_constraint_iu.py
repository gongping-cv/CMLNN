...
# 导入稀疏性版本模型
from model.CMLNN_model_Jadeite_sparsity_constraint_iu import My_LNN_based_Model_B_CLSM_Fixed3_Sparse as My_Model
...
class FixedLRExperimentConfig:
    ...
    def __init__(self,
                 ...
                 # Misc
                 name: str = 'CMLNN_sparsity_UCF101',
                 num_classes: int = 101,
    ...
    def __str__(self):
        ...
        config_str = [
            f"🔧 训练配置摘要:",
            f"  模型名称: {self.name}",
            f"  模型类型: My_LNN_based_Model_B_CLSM_Fixed3_Sparse (稀疏性约束版)",
            ...
            f"  稀疏性约束强度: {self.sparsity_lambda}",
            f"  光流来源: 从FLOW.npy采样",
...
# --------------------- Trainer ---------------------
class SparsityConstraintTrainer:
    def __init__(self, cfg: FixedLRExperimentConfig):
        ...
        # 创建模型（稀疏性版本）
        self.model = My_Model(num_classes=cfg.num_classes, num_frames=cfg.clip_len, sparsity_lambda=cfg.sparsity_lambda)
        ...
        print(f"✅ 创建稀疏性约束模型，类别数: {cfg.num_classes}")
        self.model.train()
    ...
    def _recreate_optimizer_with_stage_lr(self):
        ...
        temp_cfg = FixedLRExperimentConfig(
            ...
            sparsity_lambda=self.cfg.sparsity_lambda,
        )
    ...
    def train_one_epoch(self, loader_train):
        ...
        for step, (rgb, flow, labels) in enumerate(loader_train, start=1):
            ...
            with autocast():
                ...
                if isinstance(output, tuple):
                    if len(output) == 3:  # 稀疏性版本
                        logits, modulation_stats, sparsity_loss = output
                    elif len(output) == 2:  # 兼容其他版本
        ...
        avg_sparsity_loss = total_sparsity_loss / max(len(loader_train), 1)

        self.current_epoch += 1
        self.modulation_loss_hist.append(avg_modulation_loss)
        self.sparsity_loss_hist.append(avg_sparsity_loss)
        
        return train_loss, train_acc, avg_modulation_loss, avg_sparsity_loss
    ...
    def evaluate(self, loader_test):
        ...
        for rgb, flow, labels in loader_test:
            ...
            output = self.model(rgb, flow, reset_states=False)
            if isinstance(output, tuple):
                logits = output[0]  # 取第一个返回值（logits）
            else:
                logits = output
    ...
    def train(self, loader_train, loader_test):
        ...
        for epoch in range(1, self.cfg.num_epochs + 1):
            ...
            train_loss, train_acc, modulation_loss, sparsity_loss = self.train_one_epoch(loader_train)
            ...
            if epoch == self.cfg.num_epochs:
                print(f"Epoch {epoch:03d}/{self.cfg.num_epochs} | Time {elapsed} | "
                      ...
                      f"SparsityLoss {sparsity_loss:.6f} | "
                      f"Test {final_test_loss:.4f}({final_test_acc:.2%}) | "
                      f"Stage {self.unfreeze_stage} | LR {lr_now:.1e}")
            else:
                print(f"Epoch {epoch:03d}/{self.cfg.num_epochs} | Time {elapsed} | "
                      f"Train {train_loss:.4f}({train_acc:.2%}) | ModLoss {modulation_loss:.6f} | "
                      f"SparsityLoss {sparsity_loss:.6f} | "
                      f"Test SKIP | Stage {self.unfreeze_stage} | LR {lr_now:.1e}")
...
# --------------------- Runner ---------------------
def run_sparsity_experiment(cfg: FixedLRExperimentConfig):
    """运行稀疏性约束模型实验"""
    print("=" * 70)
    print(f"🔬 稀疏性约束模型实验 (门控机制 + 稀疏调制)")
    print("=" * 70)

    # 加载RGB和FLOW路径
    train_rgb = load_paths('train_RGB_npy_Split01_list.txt', cfg.data_root)
    train_flow = [p.replace('_RGB.npy', '_FLOW.npy') for p in train_rgb]
    ...
    trainer = SparsityConstraintTrainer(cfg)
    best_train_loss, best_test_acc = trainer.train(train_loader, test_loader)

    print(f"🎯 稀疏性约束模型结果 - 训练损失: {best_train_loss:.6f}, 测试准确率: {best_test_acc:.2%}")
...
if __name__ == "__main__":
    ...
    cfg = FixedLRExperimentConfig(
        ...
        # 模型名称
        name="CMLNN_Jadeite_sparsity_iu_UCF101_seed42",
        num_classes=101,
    ...
    try:
        final_acc, best_train_loss = run_sparsity_experiment(cfg)
        print(f"✅ 训练完成: 准确率 = {final_acc:.2%}, 训练损失 = {best_train_loss:.6f}")
        
        results_file = "CMLNN_sparsity_model_results.txt"
        with open(results_file, 'w') as f:
            f.write("稀疏性约束模型训练结果\n")
            f.write("="*50 + "\n")
            f.write(f"模型: CMLNN_model_Jadeite_sparsity_constraint_iu.py\n")
            f.write(f"稀疏性约束强度: {cfg.sparsity_lambda}\n")
            f.write(f"调制损失权重: {cfg.modulation_loss_weight}\n")
            f.write(f"配置信息:\n")
            f.write(f"- Stage 2启用: {cfg.enable_stage2}\n")
            f.write(f"- Stage 0最小轮次: {cfg.stage0_min_epochs}\n")
            f.write(f"- Stage 1最小轮次: {cfg.stage1_min_epochs}\n")
            ...
            f.write(f"Batch size: {cfg.batch_size_train}\n")
